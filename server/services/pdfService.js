...
/**
 * generateReportPDF — مولّد تقارير عام يصلح لأي قسم (مشاريع، ميزانية،
 * مخزون، موارد بشرية...). يعيد استخدام نفس طبقة عرض العربية المُختبَرة
 * بصرياً أعلاه (خط Noto Naskh + عكس ترتيب الكلمات) بدل بناء منطق عربي
 * منفصل لكل قسم جديد.
 *
 * opts = {
 *   title, subtitle, company: {name, nameEn, logo?}, docNumber, date,
 *   sections: [
 *     { heading, type:'kv',    data: { 'الحقل': 'القيمة', ... } },
 *     { heading, type:'table', columns:['العمود1','العمود2',...],
 *       rows: [[v1,v2,...], ...], widths?: [n,n,...] },
 *   ],
 * }
 */
function generateReportPDF(opts) {
  return new Promise((resolve, reject) => {
    try {
      const { title, subtitle, company, docNumber, date, sections = [] } = opts;
      const doc = new PDFDocument({ size: 'A4', margin: 40 });
      const chunks = [];
      doc.on('data', (c) => chunks.push(c));
      doc.on('end', () => resolve(Buffer.concat(chunks)));
      ...
      doc.registerFont('Arabic', ARABIC_FONT);
      const useFont = (name) => doc.font(name === 'Arabic' ? 'Arabic' : 'Helvetica');
      const writeText = (str, x, y, options = {}) => {
        if (hasArabic(String(str))) {
          useFont('Arabic').text(arabicWords(String(str)), x, y, options);
          useFont('Helvetica');
        } else {
          useFont('Helvetica').text(String(str), x, y, options);
        }
      };

      // ── ترويسة ───────────────────────────────────────────────────────
      const companyName = company?.nameEn || company?.name || 'Wassel ERP';
      useFont('Helvetica').fontSize(16).fillColor('#1a73e8').text(companyName, 40, 40);
      if (company?.name && company.name !== companyName) {
        writeText(company.name, 40, 60, { width: 260 });
      }

      useFont('Helvetica').fontSize(20).fillColor('#222').text(title, 40, 40, { align: 'right' });
      if (subtitle) doc.fontSize(10).fillColor('#666').text(subtitle, { align: 'right' });
      if (docNumber) doc.fontSize(9).fillColor('#999').text(`# ${docNumber}`, { align: 'right' });

      doc.moveTo(40, 95).lineTo(555, 95).strokeColor('#e0e0e0').stroke();
      useFont('Helvetica').fontSize(8).fillColor('#999')
        .text(`Generated: ${date ? new Date(date).toLocaleDateString('en-GB') : new Date().toLocaleDateString('en-GB')}`, 40, 100);

      let y = 120;
      const pageBottom = 770;
      const ensureSpace = (need) => { if (y + need > pageBottom) { doc.addPage(); y = 40; } };

      for (const section of sections) {
        ensureSpace(40);
        doc.rect(40, y, 515, 20).fill('#f3f2f1');
        useFont('Helvetica').fontSize(11).fillColor('#1a73e8');
        writeText(section.heading || '', 46, y + 4, { width: 500 });
        y += 28;
        ...
        if (section.type === 'kv') {
          ...
          for (const [k, v] of entries) {
            ensureSpace(18);
            useFont('Helvetica').fontSize(9).fillColor('#666');
            writeText(k, 40, y, { width: 180 });
            useFont('Helvetica').fontSize(9).fillColor('#222');
            writeText(v == null ? '-' : String(v), 230, y, { width: 325, align: 'left' });
            y += 16;
          }
          y += 10;
        } else if (section.type === 'table') {
          const cols = section.columns || [];
          const widths = section.widths || cols.map(() => Math.floor(515 / cols.length));
          const xs = []; let xAcc = 40;
          for (const w of widths) { xs.push(xAcc); xAcc += w; }

          ensureSpace(22);
          doc.rect(40, y, 515, 20).fill('#1a73e8');
          useFont('Helvetica').fontSize(8).fillColor('#fff');
          cols.forEach((c, i) => doc.text(String(c), xs[i] + 4, y + 6, { width: widths[i] - 8 }));
          y += 20;

          (section.rows || []).forEach((row, ri) => {
            ensureSpace(18);
            if (ri % 2 === 1) doc.rect(40, y, 515, 18).fill('#f8f9fa');
            row.forEach((cell, ci) => {
              const cellStr = cell == null ? '-' : String(cell);
              if (hasArabic(cellStr)) {
                useFont('Arabic').fontSize(8).fillColor('#222').text(arabicWords(cellStr), xs[ci] + 4, y + 4, { width: widths[ci] - 8 });
              } else {
                useFont('Helvetica').fontSize(8).fillColor('#222').text(cellStr, xs[ci] + 4, y + 4, { width: widths[ci] - 8 });
              }
            });
            y += 18;
          });
      ...
      useFont('Helvetica').fontSize(8).fillColor('#999').text(
        'Generated by Wassel ERP', 40, pageBottom, { width: 515, align: 'center' }
      );
...
module.exports = { generateDocumentPDF, generateReportPDF };
